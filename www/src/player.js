class Player { 
  ...
  static initialize () {
    ...
    document.addEventListener('touchstart' , (e) => {
        this.touchPoint.xs = e.touchs[0].clientX
        this.touchPoint.ys = e.touchs[0].clientY
    })
  ...
  static falling (isDownPressed) {
      ...
      if(!isBlocked) {
         // 下にブロックが無いなら自由落下してよい。プレイヤー操作中の自由落下処理をする
         ...
         if(Math.floor(this.puyoStatus.top / Config.puyoImgHeight) != y){
              ...
              y += 1;
              this.puyoStatus.y = y;
              if(y + 1 >= Config.stageRows || Stage.board[y + 1] || (y + dy + 1 >= 0 && (y + dy + 1 >= Config.stageRows || Stage.board[y + dy + 1][x + dx]))){
                  isBlocked = true;
              }
              if(!isBlocked) {
                  // 境を超えたが問題はなかった。次回も自由落下を続ける
                  this.groundFrame = 0;
                  return;
              } else {
                  // 境を超えたラブロックにぶつかった。位置を調節して、接地をを開始する。
                  this.puyoStatus.top = y * Config.puyoImgHeight;
                  this.groundFrame = 1;
                  return;
              }
          } else {
              // 自由落下で特に問題が無かった。時価も自由落下を続ける。
              this.groundFrame = 0;
              return;
          }
        }
