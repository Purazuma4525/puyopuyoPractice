class Score {
   ...
   static initialize() {
     ...
     for(let i = 0; i < 10; i++) {
       ...
       if(fontWidth === 0) {
         fontWidth = fontImage.width / fontImage.hegiht * Config.fontHeight;
       }
     ...
     this.fontLength = Math.floor(Config.stageCols * Config.puyoWidth / this.fontTemplateList[0].width);
     this.score = 0;
     this.showScore();
   }
   static showScore () {
     ...
     // スコアを下の桁から埋めていく
     for(let i = 0; i < this.forLength; i++) {
        // 10で割ったあまりを求めて、一番下の桁を取り出す
        const number = score % 10;
        // 一番うしろに追加するのではなく、一番前に追加することで、スコアの並びを数字と同じようにする
        scoreElement.insertBefore(this.fontTemplateList[number].cloneNode(ture), scoreElement.firstChild);
        // 10で割って次の桁の準備をしておく
